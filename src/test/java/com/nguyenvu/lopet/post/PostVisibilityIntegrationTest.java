 ...
 *
 * <p>Người xem có một danh tính duy nhất — id tài khoản — và bộ lọc dùng nó cho cả bốn nhánh: tác
 * giả, tư cách thành viên nhóm, đồ thị bạn bè và nội dung công khai.
 */
...
class PostVisibilityIntegrationTest extends IntegrationTestBase {
    ...
    void thanh_vien_nhom_thay_bai_trong_nhom() {
        assertThat(visibleKeys(groupMember)).containsExactly("privGroupPublic",
                "pubGroupPrivate", "pubGroupPublic", "userPublic");
    }

    @Test
    @DisplayName("người ngoài nhóm không thấy bài trong nhóm")
    void nguoi_ngoai_nhom_khong_thay_bai_nhom() {
        // groupMember là tài khoản DUY NHẤT được thêm vào nhóm.
        assertThat(visibleKeys(stranger)).doesNotContain("pubGroupPrivate", "privGroupPublic");
    }
    ...
    void tim_kiem_khong_lo_bai_rieng_tu() {
        List<Post> found = inTransaction(() ->
                postRepository.findAllVisible(stranger, "userPrivate", null));
